...
import { createSearch, loadDatasets, updatedTime, type Dataset } from './datasets'
...
export default function App() {
  ...
  const { topics, formats } = useMemo(() => {
    ...
    for (const dataset of datasets ?? []) {
      dataset.topics?.forEach((t) => topics.add(t))
      for (const r of dataset.resources) formats.set(r.format, (formats.get(r.format) ?? 0) + 1)
    }
    return {
      topics: [...topics].sort(),
      // Rare formats would only clutter the filter
      formats: [...formats].filter(([, n]) => n >= 10).sort((a, b) => b[1] - a[1]).map(([f]) => f),
    }
  }, [datasets])
  ...
  const results = useMemo(() => {
    ...
    let list = query.trim()
      ? search.search(query).map((hit) => byId.get(hit.id)!)
      : [...datasets].sort((a, b) => updatedTime(b) - updatedTime(a))
    if (topic) list = list.filter((d) => d.topics?.includes(topic))
    if (format) list = list.filter((d) => d.resources.some((r) => r.format === format))
    return list
  ...
  const visible = linked && !listed.includes(linked) ? [linked, ...listed] : listed
    ...
    <>
      <header>
        ...
        <p>
          ...
          <a href="https://opendata.dresden.de">Open-Data-Portals der Landeshauptstadt Dresden</a> durchsuchen,
          mit Karte für Geodaten.
        </p>
        <input
          type="search"
          autoFocus
        ...
        <div className="filters">
          <select value={topic} onChange={(e) => setTopic(e.target.value)} aria-label="Thema">
          ...
          {datasets && (
            <span className="count">
              {results.length} {results.length === 1 ? 'Datensatz' : 'Datensätze'}
            </span>
          )}
        </div>
      </header>
      ...
      <main>
        {error && <p className="error">Der Datensatzindex konnte nicht geladen werden: {error}</p>}
        <ul className="results">
          {visible.map((dataset) => (
            <Result
              ...
              onToggle={() => setOpenId(dataset.id === openId ? '' : dataset.id)}
            />
          ))}
        </ul>
      ...
      <footer>
        <a href="https://github.com/kiliankoe/opendata-dresden">Quellcode</a>, auch als CLI und MCP-Server. Der
        Index wird nächtlich aktualisiert. Karten von <a href="https://openfreemap.org">OpenFreeMap</a>.
      </footer>
    </>
  )
}
